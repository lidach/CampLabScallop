...
scallop_model_fun <- function(FM_flag = 1,
							effort=2,
							q=0.0003,
							U = 0,
							bag = 115,
	...
	### Age-specifc vectors ###
	  ...
	  #Bag limit stuff
	    ...
	    #from Zach citing Geiger 2006
	  	# this equation converts ash to number of scallops in a gallon
	    ash = 50 # average shell height
	...
	################################################################################
	  ...
	  nage = matrix(0,months,amax)                                                     #set up matrix
	      ...
	      #initializing first year of all year dependent values, note, these must go in a specific order
	                               #proportion of indv that spawn in each month (across ages), currently dummy values 
