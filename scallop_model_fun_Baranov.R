    ...
    for(i in 1:months){
      if(i == 1){ #Initialize
        ...
        #Bag limit
          ...
          #INSERT NEWTON-RAPHSON HERE!
          pr_F[i] <- ifelse(is.nan(pmax(0,pmin((hpue[i]/hcpue[i]),1))),0,pmax(0,pmin((hpue[i]/hcpue[i]),1))) #ratio of F in bag
        #Mortality
          F_harv[i,] <- F_init*pr_F[i] # !!!!! swap out for q_NR*et[i]
          Z[i,] <- F_harv[i,]+scenario$life.vec$M #total mortality
          catch[i] <- sum((F_harv[i,]/Z)*(1-exp(-Z))*nage[i,])
          #advance from all mortality sources (kill all oldest scallops)
          if(i!=months) nage[i+1,2:amax] <- nage[i,1:(amax-1)] * (1-exp(-Z[i,1:(amax-1)]))
